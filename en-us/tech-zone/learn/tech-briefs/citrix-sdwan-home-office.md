...
Businesses can now leverage the Citrix SD-WAN solution for Home Users. With it SD-WAN Administrator can quickly extend reliable and secure workplace environments to any location.
...
## Home User Use-Case with Citrix SD-WAN

The concerns of the WAN for the Home User use-case can be solved with the Citrix SD-WAN solution.  The bookended solution can strategically be placed on the segment of the network that typically impacts user experience the most, providing a new WAN overlay, designed to increase end-user experience and performance.
...
1.  Batch clone new sites making use of Site Profile and Templates
1.  Configure Site specific detail (e.g. device serial number, LAN interfaces and subnet to serve as the new “Remote Work Network”)
1.  Define available WAN Services (e.g. Virtual Path, Internet) as a global parameter for all sites or limit to a subset of sites
1.  Configure Application QoS and Firewall policies as a global parameter for all sites or limit to a subset of sites
1.  Finally, push the configuration changes out to production in preparation to provision the new Home User sites to join the existing SD-WAN deployment
