...
[NOTE: To log in into the SD-WAN appliance you use the name “admin” along with the password that you set here.  The username that you set here is not given admin rights.]
...
1.  On the next page (SDWAN Settings) enter:
    ...
    *  c. Management subnet – the IP address for the management interface are assigned from this
    *  d. LAN subnet - the IP address for the LAN interface are assigned from this
    *  e. WAN subnet - the IP address for the WAN interface are assigned from this
    *  f. AUX subnet - the IP address for the AUX interface are assigned from this
    [NOTE: Auto generated subnets are offered, yet can be changed as required, and AUX subnet will only have relevance in Azure SD-WAN HA]
    *  g. Route table name – name of route table object, for the local LAN, used in Azure. We leave the default name "SdWanHaRoute"
    *  h. Route Address Prefix – the address prefix of the data center on the far end for which all traffic within its scope is routed via the SD-WAN instance. This adds an Azure User Defined Route (UDR) into the local LAN routing table for local LAN routing of hosts via the Azure SD-WAN instance to reach that remote prefix. We enter 192.168.0.0/16
...
This allows the server to communicate directly with the SD-WAN instance and route through it without any other routing or tunnel constructs in Azure
...
1. Once the Citrix SD-WAN virtual machine boots select the “Log in” tab and log in with the default credentials admin/password where you are prompted to change the default password.
...
1.  Now from the SD-WAN appliance management GUI:
    *  a. Clear the initial warning regarding the license grace period for now, yet be sure to upload your permanant license as soon as possible
    *  b. On the dashboard page RECORD the appliance serial number
...
Citrix Orchestrator, hosted in Citrix Cloud, is the central service for managing and monitor Citrix SD-WAN networks. Log in to your Citrix Cloud account and request a trial or navigate to [Citrix Cloud Onboarding](https://onboarding.cloud.com/) to create a new account.
...
If you created a new Citrix Cloud account, you will receive an email with a link to log in for the first time.
...
1.  Enter WAN Link:
    ...
    *  a4. Egress and Ingress Speed – set these to 20 Mbps (Until you are able to apply your license for higher speeds.)
...
1.  Since these are new installations, we start by upgrading the Software version. From the Software Version drop-down menu select the latest version at the bottom of the list. Select “Proceed” on the popup confirming the update.
...
1.  From the Data Center Test Server open an MS-DOS prompt and verify it can traceroute to the Cloud Test Server.
...
*  Virtual Paths – if the Availability state is not green for a site verify that it can send and receive UDP traffic on port 4980. This is an essential requirement to set up virtual paths and establish the overlay network.
