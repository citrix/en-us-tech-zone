...
2.  added a Citrix-provided Podio app to act as the system of record for the Broadcast microapp and the FAQ microapp
3.  added and configured the Microapp Integration to use the Podio-backed System of Record
4.  added Subcribers to the microapps, so that appropriate people can post Broadcasts, and that everyone using Workspace will see Broadcasts and FAQs
5.  Tested the basic functionality of the Broadcast and FAQ microapps in Citrix Workspace
...
2.  Click "Get App"
3.  Use the Existing Podio workspace created in the previous step: Citrix Microapp Integrations. This will mean that both the Broadcast and FAQ Podio apps are inside the same Podio Workspace.
4.  Set access settings to Private. This workspace is used like a database, so having it private is advisable.
5.  Click "Try the app now"
...
2.  From the navigation bar, choose the Workspace associated with the Podio app that was added earlier. In this example: Citrix Microapp Integrations
...
2.  App token: Use this token to authenticate as an app rather than a user. Enter this value as an Access token parameter when you set up the integration replacing the podio_token_id variable.
3.  Client ID: The client ID is the string representing client registration information unique to the authorization server.
4.  Client Secret: The client secret is a unique string issued when setting up the target application integration.
...
4.  People who need to see FAQS in Workspace should be Subscribed to the FAQ microapp
...
2.  Go to Actions, then click Create Broadcast
3.  Compose a test Broadcast
4.  Click Create
5.  When Podio pops up asking to grant permission, choose Grant Access
...
2.  The Broadcast post is visible, and can be clicked to view the full details
...
3.  Fill in the Title and Answer (these are Required), and any other fields that are appropriate
4.  To Publish the question, set the Status to Published
...
2.  If a data sync has occured, the FAQ will appear. To force an incremental sync see [Synchronize Data](/en-us/citrix-microapps/synchronize-data.html)
3.  Click the new FAQ entry to read its full content
