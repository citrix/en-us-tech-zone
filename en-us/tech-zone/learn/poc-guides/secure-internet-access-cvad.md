...
In this Proof-of-Concept guide, you will experience the role of a Citrix administrator and you will create a connection between your organization’s CVAD deployment and Citrix Secure Internet Access.
...
*  Deploy CSIA agent (Cloud Connector) manually to VDA master image
...
#### Recommended Settings for CVAD Multi-Session OS VDA Deployments
...
| Enable Windows Desktop App:<br>(Agent will not run-on Multi-User Deployments)  | **Disable** Setting  |

#### Recommended Settings for CVAD Single-Session OS VDA Deployments
...
| Auto-Update Enabled:  | **Enable** Setting  |
| Auto-Update Release Level:  | **Level 1 - Mature**  |
| Enable Windows Desktop App:<br>(Agent will not run-on Multi-User Deployments)  | **Enable** Setting  |
...
#### Recommended Settings for CVAD Multi-Session OS VDA

| Setting  | Recommended Value  |
|---  |---  |
| Multi-User Mode:<br>(PARAM_MULTI_USER_SUPPORT)  | **(1): Enable multi-user mode.** <br>Enable to support multiple user sessions when running a terminal server. This should be enabled for terminal servers even if users are not logged in simultaneously.  |
| Terminal Server Mode:<br>(PARAM_TERMINAL_SERVER_MODE)  | **(0): Disabled** - this appears to be deprecated in favor of Multi-User Support  |
| Use Machine Name for Username:<br>(PARAM_USE_MACHINE_NAME_FOR_USERNAME)  | **(0): Disabled** - Use the user account name as the username.  |
| Redirect All Ports:<br>(PARAM_REDIRECT_ALL_PORTS)  | **(1): Enabled** - Redirect all ports to the proxy.  |
| Bypass Private Subnets:<br>(PARAM_BYPASS_PRIVATE_SUBNETS)  | **(1): Enable bypass**  |
| Captive Portal Detection:<br>(PARAM_CAPTIVE_PORTAL_DETECTION)  | **(0): Disabled**  |
| Auto-Update Enabled: (PARAM_AUTO_UPDATE_ENABLE)  | **(1): Enabled** – The cloud connector to be updated automatically.  |
| Restart After Upgrade:<br>(PARAM_RESTART_AFTER_UPGRADE)  | **(0): Disabled** - Will not prompt a restart.  |

#### Recommended Settings for CVAD Single-session OS VDA

| Setting  | Recommended Value  |
|---  |---  |
| Multi-User Mode:<br>(PARAM_MULTI_USER_SUPPORT)  | **(0): Disable multi-user mode.** <br>Enable to support multiple user sessions when running a terminal server. This should be enabled for terminal servers even if users are not logged in simultaneously.  |
| Terminal Server Mode:<br>(PARAM_TERMINAL_SERVER_MODE)  | **(0): Disabled** - this appears to be deprecated in favor of Multi-User Support  |
| Use Machine Name for Username:<br>(PARAM_USE_MACHINE_NAME_FOR_USERNAME)  | **(0): Disabled** - Use the user account name as the username.  |
| Redirect All Ports:<br>(PARAM_REDIRECT_ALL_PORTS)  | **(1): Enabled** - Redirect all ports to the proxy.  |
| Bypass Private Subnets:<br>(PARAM_BYPASS_PRIVATE_SUBNETS)  | **(1): Enable bypass**  |
...
## Deployment of the Citrix Secure Internet Access Agent to Master Image
...
2.  **Power on** the master image and log on.
3.  **Install** the appropriate CSIA Agent .msi package for your platform to the master image.  
...
5.  Select OK.
6.  Once the Group Policy is applied you will need to reboot the VDAs.

#### Scenario 2: Static VDA with user data discarded on log out

1.  On your hypervisor or cloud service provider, take a snapshot of the current VM and give the snapshot a meaningful name. This snapshot can be used to revert (roll back) machines in the catalog, if needed.
2.  In your Group Policy Management, expand your domain.
3.  Right click the location of your Master Image, then select “Link an existing GPO”.
4.  Select the Deploy CSIA Agent GPO that was created.
5.  Select OK.
6.  Once the Group Policy is applied you will need to reboot the machines.
...
### Deployed CSIA agent (Cloud Connector) manually to VDA master image
