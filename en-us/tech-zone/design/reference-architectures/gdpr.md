...
* Not only must the GDPR controller ensure the confidentiality, integrity, and availability of the personal data during its use, it must ensure that when a user exercises their right to erase that no artifacts are left behind and exposed to other apps, users, and so forth.
