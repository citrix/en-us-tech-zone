...
*  **Virtual Delivery Agent (VDA)** – All resource types rely on the VDA to delivery their content as a session. Thanks to the modular architecture VDAs can be collocated with Control components or hosted elsewhere such a public or private cloud.
...
Branch Users, on the internal network resolve the Citrix Gateway FQDN and their path depend on the Enterprise topology. If an internal Citrix Gateway is hosted in the Data Center, their internal DNS resolves to its private IP address and they are routed by their local SD-WAN appliance over a virtual path on the overlay network.

Otherwise if only an external Citrix Gateway exists the local SD-WAN appliance can be configured to breakout the connection directly to the internet. The SD-WAN appliance can have an internet service configured, DNS forwarder, and business application rule to direct the connection to the Citrix Gateway via the Internet.
...
Focusing on the goal of the document to provide guidance on optimal use of Citrix SD-WAN, with Citrix Gateway, in a Citrix Virtual Apps and Desktop environment we discuss pertinent hardware, software, and key configuration elements.
