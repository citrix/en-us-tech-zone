...
* Full Mesh – full mesh clusters include support for a variety of microservices that need east-west communication between microservices within the cluster and north-south communication outside of the cluster.
* Service Mesh Lite- an Ingress solution typically performs L7 proxy functions for north-south (N-S) traffic. The Service Mesh lite architecture uses the same Ingress solution to manage east-west traffic and overcome limitations of Kubernetes built-in service.
...
Based on the preceding requirements, CompanyA created the following high-level conceptual architecture. This architecture meets all of the initial requirements while giving CompanyA the foundation to expand to additional use cases in the future.
