...
On the subject of zones, implemented using [zone preference](/en-us/xenapp-and-xendesktop/7-15-ltsr/manage-deployment/zones.html#zone-preference)(7.11 and later) represents another high-availability design option which does not constitute DR but solves challenges for multi-region deployments which do not require a fully-redundant DR solution. As illustrated in the following diagram, identical Citrix resources are deployed accross several zones and aggregated into a single Delivery Group. Zone preference (with ability to fail back to other zones for a given resource) can be controlled based on application, user's home zone, or user location. Please refer to [Zone Preference Internals](https://www.citrix.com/blogs/2017/04/17/zone-preference-internals/) for more information on this subject. Note that StoreFront Server Groups are present in each zone location, as is recommended in order for users to continue accessing resources from their local access tier in event of the primary zone being unavailable.
...
Continuing on the HA and zones theme, the following diagram illustrates another multi-zone architecture which leverages the [VDA Registration](/en-us/citrix-virtual-apps-desktops/manage-deployment/vda-registration.html) auto-update feature introduced in XenDesktop 7.6 which, in the context of zones, will allow VDAs to maintain a locally cached up-to-date list of all Controllers within a Site, allowing them to failover registration to VDAs in another zone such as the primary zone in event their local Delivery Controllers or Cloud Connectors become unavailable.
