...
![nFactor_persist_auth_wide](https://user-images.githubusercontent.com/16384254/124257832-0c38ea80-db2d-11eb-9716-0dca55884edf.png)
**Figure 1: Logic flow of "trust this device" functionality**

At the end of this page there is an appendix with links to additional resources (Example ns.conf files, LoginSchema) which you can use and edit to suit your environment.

### nFactor loginSchema
The look and feel of a login interface is important for
a business to provide a more intuitive user experience. The Citrix ADC nFactor authentication framework is designed to be flexible and extensible with a clear separation of colors, backgrounds and themes in portal themes and custom login labels, text fields, radio buttons, login credentials as well in loginSchemas to fully customize the user interface display.
...
In Citrix ADC software version 13.0 a new feature, nFactor Flow, was introduced that visualizes the logic
of policy labels. This allows visual configuration of authentication flows, which is easier to follow (Figure 4).
...
``` 
add ns assignment assign_gvar_aaa_ persistusers -variable ""$gvar_aaa_persist_cookie_users[HTTP.REQ.BODY(500).AFTER_STR(\"enable_persist=\")]" -set 1
add rewrite policy pol_rw_set_gvar_ aaapersistusers "http.req.url.path.get(1).eq(\"setvar\")" assign_gvar_aaa_ persistusers
``` 
...
add authentication policy pol_auth_noauth_enable_persist -rule "HTTP.REQ. BODY(500).AFTER _ STR(\"persistdevice=\"). STARTSWITH(\"true\")" -action act_auth_ webauthpersist
bind authentication policylabel nf_Radius__nf_nFlowTrust -policyName
pol_auth_noauth_enable_persist -priority 100 -gotoPriorityExpression next -nextFactor nf_RadiusPassthru__nf_nFlowTrust
...
### Citrix Gateway with nFactor
The nFactor configuration built here, including the “trust this device” check box, can be linked to a Citrix Gateway virtual server as well. For this, a Citrix ADC Advanced or Premium license is required as **nFactor is not supported with the Citrix ADC Standard Edition nor the Citrix Gateway Advanced VPX licenses.**
...
https://docs.citrix.com/en-us/citrix-adc/13/aaa-tm/multi-factor-nfactor-authentication.html
...
https://docs.citrix.com/en-us/citrix-adc/13/appexpert/policies-and-expressions/advanced-policy-exp-evaluation-text/complex-text-operations.html

## Appendix
* SingleAuthPersist.xml
* ns_conf_Snip.conf
